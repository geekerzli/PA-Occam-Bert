...
import os
build_path = os.path.join(os.path.dirname(os.path.abspath(
    __file__)), './lib')
...
def embedding_postprocessor(input_tensor,
    ...
    if use_token_type:
        ...
        one_hot_ids = tf.one_hot(
            flat_token_type_ids, depth=token_type_vocab_size, dtype=tf.flags.FLAGS.floatx)
...
def attention_layer(from_tensor,
    ...
    print("size_per_head",size_per_head)
...
def transformer_model(input_tensor,
    ...
    for layer_idx in range(num_hidden_layers):
        with tf.variable_scope("layer_%d" % layer_idx):
            ...
            if tf.flags.FLAGS.use_fasterTF:
